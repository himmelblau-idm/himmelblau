...
use crate::dag::{parse_dag_message, DagInfo};
...
pub(super) struct Updates {
    ...
    /// Whether DAG browser authentication is active
    pub(super) dag_active: bool,
    /// The current DAG authentication info (URL and code)
    pub(super) dag_info: Option<DagInfo>,
}
...
pub struct Greeter {
    ...
    /// WebView for DAG browser authentication (created lazily)
    pub(super) dag_web_view: Option<webkit6::WebView>,
}
...
impl Greeter {
    ...
    pub(super) async fn cancel_click_handler(&mut self) {
        ...
        self.updates.set_dag_active(false);
        self.updates.set_dag_info(None);
            ...
            Response::AuthMessage {
                ...
                match auth_message_type {
                    ...
                    AuthMessageType::Info => {
                        ...
                        // e.g.: asking for a fingerprint, or a DAG authentication URL
                        info!("greetd sent an info: {auth_message}");
                        self.updates.set_input_mode(InputMode::None);

                        // Check if this is a DAG (Device Authorization Grant) message
                        if let Some(dag_info) = parse_dag_message(&auth_message) {
                            info!(
                                "DAG authentication detected: url={}, code={:?}",
                                dag_info.url, dag_info.code
                            );
                            self.updates.set_dag_active(true);
                            self.updates.set_dag_info(Some(dag_info));
                            // Set a user-friendly message
                            self.updates.set_message(
                                "Complete authentication in the browser below".to_string(),
                            );
                        } else {
                            // Not a DAG message, display as normal
                            self.updates.set_dag_active(false);
                            self.updates.set_dag_info(None);
...
impl Greeter {
    /// Create and configure a WebView for DAG authentication.
    pub(super) fn create_dag_web_view(&mut self) -> webkit6::WebView {
        use webkit6::prelude::*;
        ...
        // Set size
        web_view.set_size_request(500, 500);
        web_view.set_hexpand(true);
        web_view.set_vexpand(true);

        self.dag_web_view = Some(web_view.clone());
        web_view
    }

    /// Navigate the DAG WebView to the specified URL.
    pub(super) fn navigate_dag_browser(&self, url: &str) {
        use webkit6::prelude::WebViewExt;

        if let Some(web_view) = &self.dag_web_view {
            info!("Navigating DAG browser to: {}", url);
