...
impl AsyncComponent for Greeter {
    ...
    view! {
        ...
        gtk::ApplicationWindow {
            ...
            Ui {
                ...
                // DAG browser bindings
                #[template_child]
                dag_browser_frame {
                    #[track(model.updates.changed(Updates::dag_active()))]
                    set_visible: model.updates.dag_active,
                },
                #[template_child]
                dag_code_label {
                    #[track(model.updates.changed(Updates::dag_info()))]
                    set_label: model.updates.dag_info.as_ref()
                        .and_then(|info| info.code.as_deref())
                        .unwrap_or(""),
                    #[track(model.updates.changed(Updates::dag_info()))]
                    set_visible: model.updates.dag_info.as_ref()
                        .map(|info| info.code.is_some())
                        .unwrap_or(false),
                },
                #[template_child]
                dag_copy_button {
                    ...
                    set_visible: model.updates.dag_info.as_ref()
                        .map(|info| info.code.is_some())
                        .unwrap_or(false),
                    connect_clicked => Self::Input::CopyDagCode,
                },
            }
        }
    }
    ...
    fn post_view() {
        ...
        // Navigate DAG browser when dag_info changes
        if model.updates.changed(Updates::dag_info()) {
            if let Some(dag_info) = &model.updates.dag_info {
                model.navigate_dag_browser(&dag_info.url);
            }
        }
    }
    ...
    async fn init(
        ...
        // Create and add the DAG WebView to the browser container
        let dag_web_view = model.create_dag_web_view();
        widgets.ui.dag_browser_box.append(&dag_web_view);
    ...
    async fn update(
        ...
        match msg {
            ...
            Self::Input::CopyDagCode => {
                if let Some(dag_info) = &self.updates.dag_info {
                    if let Some(code) = &dag_info.code {
