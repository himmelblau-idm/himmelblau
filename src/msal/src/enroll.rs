...
use base64::Engine;
use hostname;
use kanidm_hsm_crypto::{LoadableIdentityKey, MachineKey};
use openssl::rsa::Rsa;
...
pub async fn register_device(
    ...
    access_token: &str,
    domain: &str,
    tpm: &mut (dyn kanidm_hsm_crypto::Tpm + Send),
    ...
    // Create the CSR
    let csr_der =
        match tpm.identity_key_certificate_request(machine_key, certificate_id_key, domain) {
            Ok(csr_der) => csr_der,
            Err(_e) => return Err(anyhow!("Failed creating CSR")),
        };

    // Create the transport key
    ...
    if resp.status().is_success() {
        ...
        if let Err(_e) = tpm.identity_key_load(machine_key, &loadable_id_key) {
            return Err(anyhow!("Failed loading id key"));
        }
