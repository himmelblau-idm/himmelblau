...
use himmelblau::EntraSshCertificate;
    ...
    I: IdProvider + Sync,
{
    ...
    /// Acquire an Entra OpenSSH certificate for a previously resolved user.
    /// The provider enforces that the cached credential is a sealed PRT.
    pub async fn acquire_ssh_certificate(
        &self,
        account_id: &str,
        openssh_public_key: &str,
    ) -> Result<EntraSshCertificate, ()> {
        let token = self
        ...
        match result {
            ...
            Err(err) => {
                warn!(
                    ?err,
                    account_id = token.spn,
                    "SSH certificate acquisition denied"
                );
                Err(())
            }
        }
    ...
    pub async fn pam_account_allowed(&self, account_id: &str) -> Result<Option<bool>, ()> {
        let Some(token) = self.get_usertoken(Id::Name(account_id.to_string())).await? else {
            // An empty pam_allow_groups list means "all known Himmelblau
            // users", not "all strings supplied by a caller".  This is
            // especially important for SSH certificate authorization, where
            // an unknown target must fail closed.
            return Ok(None);
        };
        ...
        if self.pam_allow_groups.is_empty() {
            // An empty allow list permits every known user with a valid token.
            Ok(Some(true))
        } else {
            Ok(Some({
                ...
                // only for deterministic v5 group UUIDs generated by
                // Himmelblau, not for Entra object-id groups.
                let user_set: BTreeSet<_> = token
                    .groups
                    .iter()
                    .flat_map(|g| {
                        let mut ids = vec![g.uuid.hyphenated().to_string()];
                        if g.uuid.get_version() == Some(Version::Sha1) {
                ...
                debug!("User has valid token: {}", token.valid);

                intersection_count > 0 && token.valid
            }))
        }
    }
