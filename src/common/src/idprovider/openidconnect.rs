...
impl Drop for OrchestratorProvidedInput {
    fn drop(&mut self) {
        self.value.zeroize();
    }
}
...
// plus extensive token bundles (multiple JWTs @ ~8KB each), session logs (64 entries
// @ ~512 bytes), and metadata. Server enforces 384KB limit; client matches to avoid
// rejecting valid responses. This limit prevents DoS while allowing legitimate large
// responses during complex authentication flows.
const MAX_ORCHESTRATOR_RESPONSE_BYTES: usize = 384 * 1024;
        ...
        if src.len() > MAX_ORCHESTRATOR_RESPONSE_BYTES {
            ...
            return Err(std::io::Error::new(
                ...
                format!(
                    ...
                    MAX_ORCHESTRATOR_RESPONSE_BYTES, frame_len
                ),
            ));
        }
...
impl Encoder<OrchestratorCommand> for OrchestratorCodec {
    ...
    fn encode(&mut self, msg: OrchestratorCommand, dst: &mut BytesMut) -> Result<(), Self::Error> {
        let data = Zeroizing::new(serde_json::to_vec(&msg).map_err(|err| {
            std::io::Error::new(
                std::io::ErrorKind::InvalidData,
...
impl OidcProvider {
    ...
    async fn maybe_start_orchestrator_flow(
        ...
        let issuer_url = self.config.lock().await.get_oidc_issuer_url();
                    ...
                    OidcMfaExtraData::Orchestrator { state } => {
                        ...
                        match orchestrator_continue(&cfg, &state, provided_inputs).await {
                            ...
                            Err(_) => {
                                ...
                                if let Some(dag_json) = state.dag_json.clone() {
                                    warn!(
                                        session_id = %state.session_id,
                                        "Falling back to DAG polling after orchestrator failure"
                                    );
                                    *extra_data = Some(serialize_oidc_mfa_extra_data(
                                        &OidcMfaExtraData::DeviceFlow { dag_json },
                                    )?);
                                    flow.polling_interval =
                                        Some(cfg.poll_interval_secs.saturating_mul(1000));
