...
impl IdProvider for HimmelblauProvider {
    ...
    async fn unix_user_online_auth_step<D: KeyStoreTxn + Send>(
        ...
        // Issue #1206: When the network is down during reauth initiation, fall
        // back to offline unlock using the cached token, but only after
        // defensively re-validating the Hello PIN to prevent bypass.
