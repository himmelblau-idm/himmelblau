...
impl IdProvider for HimmelblauMultiProvider {
    ...
    async fn check_online(&mut self, tpm: &mut tpm::BoxedDynTpm, now: SystemTime) -> bool {
        for (_domain, provider) in self.providers.read().await.iter_mut() {
        ...
        match account_id {
            Some(account_id) => match split_username(account_id) {
                Some((_sam, domain)) => {
                    let providers = self.providers.read().await;
                    match providers.get(domain) {
                        Some(provider) => provider.get_cachestate(Some(account_id)).await,
                        None => CacheState::Offline,
                    }
                }
                None => CacheState::Offline,
            },
            None => {
                for (_domain, provider) in self.providers.read().await.iter_mut() {
                    match provider.get_cachestate(None).await {
                        ...
                        CacheState::OfflineNextCheck(time) => return CacheState::OfflineNextCheck(time),
                        _ => continue,
                    }
                }
            }
