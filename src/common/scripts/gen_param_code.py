...
def generate_man_page(params: List[Parameter], sections: Dict[str, Section]) -> str:
    ...
    lines.append('.B /usr/lib/himmelblau/himmelblau.conf')
    lines.append('.br')
    lines.append('.B /run/himmelblau/himmelblau.conf')
    lines.append('.br')
    ...
    lines.append('Changes to any of the configuration files described below')
    ...
    # Configuration hierarchy and drop-ins
    lines.append('.SH CONFIGURATION PRECEDENCE')
    lines.append('Himmelblau looks for the main configuration file in the following locations, from lowest to highest priority:')
    ...
    lines.append('.B /etc/himmelblau/himmelblau.conf')
    lines.append('.P')
    lines.append('Only the highest-priority main file that exists is parsed. An empty higher-priority file or a symbolic link to')
    lines.append('.B /dev/null')
    lines.append('masks lower-priority main files.')
    lines.append('.P')
    lines.append('Drop-in files ending in')
    lines.append('.B .conf')
    lines.append('are read from directories formed by appending')
    lines.append('.B .d')
    lines.append('to each path above, for example')
    lines.append('.BR /etc/himmelblau/himmelblau.conf.d/ .')
    lines.append('Files with the same basename override each other according to the hierarchy priority. The remaining files are parsed in lexicographic basename order after the main file, so later names have higher priority. Empty files and symbolic links to')
    lines.append('.B /dev/null')
    lines.append('mask same-named drop-ins at lower priorities. Drop-ins are still parsed when the main file is masked, and drop-in directories are not recursive.')
    lines.append('')
