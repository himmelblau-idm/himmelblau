...
impl Idmap {
    ...
    pub fn gen_to_unix(&self, tenant_id: &str, input: &str) -> Result<u32, IdmapError> {
        ...
        let range_size = inclusive_range_size(*range)?.get();

        let input = normalize_for_hash(input);
        let hash = murmur3_32(&input)?;
        compute_gen_id(*range, hash % range_size)
    }
...
fn inclusive_range_size(range: DomainRange) -> Result<NonZeroU32, IdmapError> {
    ...
    NonZeroU32::new(range_size).ok_or(IDMAP_NO_RANGE)
}

fn compute_gen_id(range: DomainRange, offset: u32) -> Result<u32, IdmapError> {
    let id = range.min.checked_add(offset).ok_or(IDMAP_NO_RANGE)?;
    if id == u32::MAX {
...
mod tests {
    ...
    fn gen_to_unix_degenerate_ranges() -> Result<(), Box<dyn std::error::Error>> {
        for (range, expected) in [
            ...
            ((1000, 1001), Ok(1001)),
            ((1000, 1002), Ok(1002)),
            ((u32::MAX, u32::MAX), Err(IDMAP_NO_RANGE)),
            ((2000, 1000), Err(IDMAP_NO_RANGE)),
            ((0, u32::MAX), Err(IDMAP_NO_RANGE)),
        ] {
            let mut idmap = Idmap::new()?;
    ...
    #[test]
    fn object_id_to_unix_range_boundaries() -> Result<(), Box<dyn std::error::Error>> {
        let sid = AadSid::from_sid_str("S-1-12-1-4294967295")?;

        for (range, expected) in [
            ((100, 100), Err(IDMAP_NO_RANGE)),
            ((u32::MAX, u32::MAX), Err(IDMAP_NO_RANGE)),
            ((2000, 1000), Err(IDMAP_NO_RANGE)),
            ((0, u32::MAX), Ok(0)),
        ] {
            ...
            idmap.add_gen_domain("domain", "tenant", range)?;
            assert_eq!(
                idmap.object_id_to_unix_id("tenant", &sid),
                expected,
                "Mapping result for range {range:?} did not match"
            );
        }
