                ...
                async {
                    ...
                    .await
                    {
                        ...
                        Err(ApplyPolicyError::NonCompliant(rules)) => {
                            debug!(
                                count = rules.len(),
                                "compliance check: device is non-compliant for {}", account_id
                            );
...
fn should_start_intune_policy_application(
    ...
    match throttle_state.get(account_id) {
        ...
        _ => {
            throttle_state.insert(account_id.to_string(), IntunePolicyThrottleState::InFlight);
            true
        }
    }
}
