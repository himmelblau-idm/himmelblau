...
def http_request(
    ...
    *,
    ...
    raise RuntimeError(f"HTTP request failed for {url}: retry loop ended unexpectedly")
...
def package_url(tag: str, *parts: str) -> str:
    ...
    if parts and (parts[-1] == "" or parts[-1].endswith("/")):
        path += "/"
    return urllib.parse.urljoin(stable_release_url(tag), path)


def safe_metadata_path(path: str) -> str | None:
    ...
    decoded_segments = [urllib.parse.unquote(segment) for segment in normalized.split("/")]
...
def parse_debian_packages(text: str, base_url: str, tag: str, distro: str, github_repo: str) -> list[Artifact]:
    ...
    for block in re.split(r"\n\s*\n", text.strip()):
        ...
        if not filename or not sha256 or not name:
            continue
        safe_filename = safe_metadata_path(filename)
        if safe_filename is None:
            eprint(f"Warning: skipping unsafe Debian Filename for {tag}/{distro}: {filename}")
            continue
        ...
        artifacts.append(
            Artifact(
                ...
                artifact_url=urllib.parse.urljoin(base_url, safe_filename),
...
def parse_rpm_primary(xml_bytes: bytes, base_url: str, tag: str, distro: str, github_repo: str) -> list[Artifact]:
    ...
    for package in root.findall("common:package", ns):
        ...
        if not sha256 or checksum.attrib.get("type") != "sha256" or not href:
            continue
        safe_href = safe_metadata_path(href)
        if safe_href is None:
            eprint(f"Warning: skipping unsafe RPM href for {tag}/{distro}: {href}")
            continue
        ...
        artifacts.append(
            Artifact(
                ...
                version=version,
                digest=f"sha256:{sha256}",
                artifact_url=urllib.parse.urljoin(base_url, safe_href),
                registry_url=PACKAGE_BASE_URL,
...
def discover_rpm_artifacts(tag: str, github_repo: str) -> list[Artifact]:
    ...
    for distro in distros:
        ...
        safe_primary_href = safe_metadata_path(primary_href)
        if safe_primary_href is None:
            eprint(f"Warning: skipping unsafe RPM primary metadata href for {tag}/{distro}: {primary_href}")
            continue
        primary_bytes = fetch_bytes(urllib.parse.urljoin(distro_url, safe_primary_href))
        if safe_primary_href.endswith(".gz"):
...
def local_compare_context(previous_tag: str, tag: str) -> str:
    commit_log = run_git(
        [
            "log",
            f"--max-count={MAX_COMMITS_IN_CONTEXT}",
            "--date=short",
