...
installs policy files that automatically configure Chrome/Chromium to
install the required browser extension for SSO functionality.
...
{generate_files_section(sso_policies_metadata, name="himmelblau-sso-policies", dirs=["/etc/chromium/policies", "/etc/chromium/policies/managed", "/etc/opt/chrome", "/etc/opt/chrome/policies", "/etc/opt/chrome/policies/managed", "%attr(0555,root,root) %{chrome_policy_dir}", "%attr(0555,root,root) %{chromium_policy_dir}"])}
